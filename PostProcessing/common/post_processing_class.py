...
class PostProcess:
    ...
    def save_as_svg(self, name, metric_value, metric_name='metric_name', formula='', unit=''):
        y_min = metric_value
        y_max = metric_value
        ts_variable = self.data_array(name)
        if min(ts_variable) < y_min:
            y_min = min(ts_variable)
        if max(ts_variable) > y_max:
            y_max = max(ts_variable)
        metric_array = np.ones(len(self.time)) * metric_value
        plt.plot(self.time, ts_variable)
        ...
        for metric_item in sum_rep_json['Metrics']:
            if metric_item['Name'] == metric_name and 'VisualizationArtifacts' in metric_item:
                artifact = {'Location': plot_path.replace(os.path.sep, '/'), 'Tag': name}
                metric_item['VisualizationArtifacts'].append(artifact)
    ...
    def get_time_for_crossing(self, name, value, start_index=0, end_index=-1):
        """
        Gets the time of the crossing of value.
        ...
        cnt = 0
        start_sign = 1
        if data[start_index] - value < 0:
            start_sign = -1

        for x in data:
            diff = x - value
            if diff * start_sign < 0:
                index = cnt
                break
            else:
                cnt += 1
        ...
        if index >= 0:
            return (self.time[start_index + index] + self.time[start_index + index - 1]) / 2
    ...
    def integrate(self, name):
        """
        Returns the area under the curve (with sign) of the time-series for the variable
        name - name of variable
        """
    ...
    def integrate_abs(self, name):
        """
        Returns the total area under the curve of the (abs of) time-series for the variable
        name - name of variable
        """
...
def check_limits_and_add_to_report_json(pp, limit_dict):
    ...
    for limit_item in limit_dict['LimitChecks']:
        ...
        limit_name = limit_item['LimitName']
        ...
        if limit_type == 'min':
            ...
            limit_array = np.ones(len(pp.time)) * limit_value
            plot_limit(pp, [limit_array], limit_name, limit_type, modelica_uri)
            ...
            limit_array_max = np.ones(len(pp.time)) * limit_value
            limit_array_min = np.ones(len(pp.time)) * limit_value * -1
            plot_limit(pp, [limit_array_max, limit_array_min], limit_name, limit_type, modelica_uri)
...
def plot_limit(pp, limit_arrays, limit_name, limit_type, modelica_uri):

        y_min = np.min(pp.data_array(modelica_uri))
        y_max = np.max(pp.data_array(modelica_uri))
        plt.plot(pp.time, pp.data_array(modelica_uri))
        for limit_array in limit_arrays:
            plt.plot(pp.time, limit_array, 'r')
            if np.max(limit_array) > y_max:
                y_max = np.max(limit_array)
            if np.min(limit_array) < y_min:
                y_min = np.min(limit_array)
        plt.plot()
        plt.title('{0}\n{1}({2})'.format(limit_name, limit_type, modelica_uri))
        plt.xlabel('time\n[s]')
        plt.ylabel('{0}'.format(modelica_uri))
        delta = (y_max - y_min) * 0.05
        if delta == 0:
            delta = 1
        plt.ylim([y_min - delta, y_max + delta])
        if not os.path.isdir('plots'):
            os.mkdir('plots')
        plot_path = os.path.join('plots', '{0}_{1}.svg'.format(limit_name, modelica_uri))
        plt.savefig(plot_path)
        plt.close()
        ...
        sum_rep_json['VisualizationArtifacts'].append({'Location': plot_path.replace(os.path.sep, '/'),
                                                       'Tag': limit_name})
